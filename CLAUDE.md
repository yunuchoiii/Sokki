...
- **아직 아무도 확인하지 못한 것 셋.** 0.6.0~0.6.1 에 실려 나갔는데 여전히 미확인이다.
  ① 로그인 시 자동 실행이 실제로 시스템 설정의 로그인 항목에 등록되는지(화면 렌더로만 봤다)
  ② 붙여넣기가 Chrome·ChatGPT 에서도 되는지(Orca 에서만 확인했다)
  ③ 팝오버 핵심 요약 토글의 **켜진** 색이 코랄인지(미리보기가 꺼진 상태만 그려서 못 봤다.
     파랗게 남아 있으면 `.tint` 가 안 먹은 것이니 설정 창의 `InkToggle` 처럼 직접 만든 토글로 바꾼다)
