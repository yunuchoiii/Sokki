...
- **0.7.1 이 자동 업데이트의 첫 실전이다.** 0.7.0 이 깔린 맥에서 창이 스스로 뜨는지, "설치"만 눌러
  교체·재실행까지 가는지 **사용자에게 확인을 부탁할 것.** 여기까지 확인되면 이 줄을 지운다.
  0.7.0 에서 확인된 것: 앱이 피드를 실제로 읽고(`SULastCheckTime`), 배포된 appcast 의 서명이 DMG 와
  맞고(`sign_update --verify`, 틀린 서명은 거부), 같은 버전일 때 "최신 버전입니다" 창이 떴다.
  **더 높은 버전을 만나는 경로만 안 밟혔다.**
- ⚠️ **앱 UI 를 영어로도 낼 생각이면 `Contents/Resources/ko.lproj` 빈 폴더를 지우지 말 것**(build.sh 가 만든다).
  `.lproj` 가 하나도 없으면 macOS 가 앱을 영어 앱으로 보고 프로세스 언어를 영어로 고정해서,
  Sparkle 이 ko 번역을 가지고 있어도 업데이트 창이 영어로 뜬다(0.7.0 에서 그랬다). en 을 더하되 ko 는 남긴다.
