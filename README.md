...
- 새 버전이 나오면 실행할 때 알려 줍니다(하루 한 번 확인). 설정 > 업데이트 탭에서 바로 확인할 수도 있습니다.
  다운로드를 누르면 DMG 를 받고, 열어서 Applications 에 끌어 넣으면 됩니다.
...
| 음성인식 · AI | 애플 서버 인식 여부(기본 켬), AI 모델, 정리 스타일(기본·격식·구어체·원문 최소 손질), API 키 |
| 단축키 | 직접 녹음 또는 프리셋 |
...
- 음성은 애플 음성 인식으로 처리합니다. 기본은 애플 서버 인식(더 정확)이고, 설정 > 음성인식 · AI 에서 "애플 서버에서 처리"를 끄면 인터넷 없이 이 맥 안에서만 인식합니다.
...
Sources/
  ...
  SettingsWindow.swift  설정 창 (일반 / 개인화 / 음성인식 · AI / 단축키 / 업데이트 / 고급 · 진단)
