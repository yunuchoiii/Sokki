...
enum Prompts {
    ...
    static func userMessage(_ raw: String, style: PolishStyle) -> String {
        ...
        "~하면 돼(요)", "~해 주시면 돼요"는 방법·조건을 알려 주는 평서문이다. 물음표를 붙이거나 "될까요"로 바꾸지 않는다. \
        "~봤어", "~했어" 뒤에 그 일에 대한 내 의견·결과가 이어지면 내가 한 일을 알리는 평서문이다.
    ...
    private static let summaryBase = """
        ...
        - 받아쓰기는 물음표를 빠뜨리고 마침표를 찍는다. 반말 질문은 평서문과 모양이 같다("못 하는 거야.", "못 해.", \
          "되는 거야."). "그럼", "혹시", "왜"로 시작하거나 앞뒤 문맥상 상대에게 확인을 구하는 말이면 질문으로 남긴다 \
          ("테스트는 못 하는지 질문"). 질문을 단정("테스트 불가함")으로 바꾸면 뜻이 뒤집힌다. 애매하면 단정하지 않는다.
    ...
    private static var base: String {
        ...
        - 물음표 살리기: 받아쓰기는 물음표를 거의 안 찍는다. "~야?", "~돼?", "~거야?", "~못 해?", \
          ...
          평서문으로 바꿔 놓으면 질문이 사라져 뜻이 달라진다. 거꾸로 "~하면 돼(요)", "~해 주시면 돼요"처럼 \
          방법·조건을 알려 주는 말은 평서문이다. "될까요?"로 바꾸면 알려 주던 말이 묻는 말이 된다. \
          "~봤어", "~했어"처럼 한 일을 말한 뒤 그 일에 대한 내 의견·결과가 이어지면("PR 봤어. 로직은 괜찮은데…") \
          내가 한 일을 알리는 말이다. "봤어?"로 바꾸면 내가 본 게 상대에게 묻는 말이 된다.
