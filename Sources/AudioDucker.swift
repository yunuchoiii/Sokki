...
enum AudioDucker {
    ...
    static func duck(device: AudioDeviceID? = nil) {
        ...
        // 볼륨 속성이 없는 출력(HDMI·DisplayPort). ⏯ 미디어 키는 "지금 재생 중" 앱이 없으면 macOS 가 음악 앱을 열어 버려서 쓰지 않는다.
        Log.write("소리 줄이기 불가 — \(name(of: dev)) 은 볼륨 조절이 안 되는 출력" + (isRunningSomewhere(dev) ? " (소리 나는 중)" : ""))
    }
